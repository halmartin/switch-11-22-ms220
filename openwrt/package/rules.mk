...
$(PKG_BUILD_DIR)/.prepared: $(DL_DIR)/$(PKG_SOURCE)
	...
	if [ -n "$(PKG_UNTAR_DIR)" ]; then \
		tar -C $(PKG_UNTAR_DIR) -cpf - . | tar -C $(PKG_BUILD_DIR) -xpf -; \
		rm -rf $(PKG_UNTAR_DIR); \
	fi
	if [ -d ./patches/$(PKG_VERSION) ]; then \
		rm -f ./patches/*~; \
		$(PATCH) $(PKG_BUILD_DIR) ./patches/$(PKG_VERSION) ; \
	elif [ -d ./patches ]; then \
		rm -f ./patches/*~; \
...
.PHONY: all source prepare compile install clean version
