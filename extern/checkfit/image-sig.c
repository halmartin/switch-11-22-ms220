		...
		char **err_msgp)
{
	...
	info->fdt_blob = signature_fdt;
	info->required_keynode = required_keynode;
	out("%s:%s", algo_name, info->keyname);
	...
	*no_sigsp = 1;
	...
	fdt_for_each_subnode(noffset, sig_blob, sig_node) {
		...
		ret = fit_image_verify_sig(fit, image_noffset, data, size,
					sig_blob, noffset);
		if (ret) {
			out("Failed to verify required signature '%s'\n",
			 ...
			 char **err_msgp)
{
	...
	debug("%s: fdt=%p, conf='%s', sig='%s'\n", __func__, signature_fdt,
	      fit_get_name(fit, noffset, NULL),
	      fit_get_name(signature_fdt, required_keynode, NULL));
	*err_msgp = NULL;
	if (fit_image_setup_verify(&info, fit, noffset, required_keynode,
				   err_msgp))
		return -1;
...
{
	return fit_config_verify_sig(fit, conf_noffset, NULL, -1);
}
