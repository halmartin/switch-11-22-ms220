...
static struct amc6821_data *amc6821_update_device(struct device *dev)
{
	...
	if (time_after(jiffies, data->last_updated + timeout) ||
		...
		reg = i2c_smbus_read_byte_data(client, AMC6821_REG_CONF2);
		data->tach_enable = !!(reg & AMC6821_CONF2_TACH_EN);
		data->tach_mode = !!(reg & AMC6821_CONF2_TACH_MODE);
...
static ssize_t get_tach_enable(
		struct device *dev,
		struct device_attribute *devattr,
		char *buf)
{
		...
		size_t count)
{
	...
	mutex_lock(&data->update_lock);
	data->tach_enable = clamp_val(val, 0, 1);
		...
		size_t count)
{
	...
	mutex_lock(&data->update_lock);
	data->tach_mode = clamp_val(val, 0, 1);
	config = i2c_smbus_read_byte_data(client, AMC6821_REG_CONF2);
...
static SENSOR_DEVICE_ATTR(tach_enable, S_IWUSR | S_IRUGO,
	get_tach_enable, set_tach_enable, 0);
static SENSOR_DEVICE_ATTR(tach_mode, S_IWUSR | S_IRUGO,
	get_tach_mode, set_tach_mode, 0);
...
static struct attribute *amc6821_attrs[] = {
	...
	&sensor_dev_attr_tach_enable.dev_attr.attr,
	&sensor_dev_attr_tach_mode.dev_attr.attr,
	NULL
};
