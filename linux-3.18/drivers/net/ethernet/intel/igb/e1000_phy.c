...
s32 igb_read_phy_reg_mdic(struct e1000_hw *hw, u32 offset, u16 *data)
{
	...
	mdic = ((offset << E1000_MDIC_REG_SHIFT) |
		...
		(hw->mdio_use_comp ? E1000_MDIC_INT_EN : 0) |
		(E1000_MDIC_OP_READ));
	...
	if (hw->mdio_use_comp) {
		down(&hw->mdio_sema);
		if (!wait_for_completion_timeout(&hw->mdio_comp, usecs_to_jiffies(3 * E1000_GEN_POLL_TIMEOUT * 50))) {
			hw_dbg("MDIO read timeout\n");
		}
