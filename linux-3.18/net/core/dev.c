...
int __netif_receive_skb(struct sk_buff *skb, __be16 type, int notifier_data)
{
	int ret;
	...
	if (sk_memalloc_socks() && skb_pfmemalloc(skb)) {
		...
		ret = __netif_receive_skb_core(skb, true, type, notifier_data);
		tsk_restore_flags(current, pflags, PF_MEMALLOC);
	} else
		ret = __netif_receive_skb_core(skb, false, type, notifier_data);
...
#endif
	ret = __netif_receive_skb(skb, skb->protocol, 0);
	rcu_read_unlock();
	return ret;
}
...
static int process_backlog(struct napi_struct *napi, int quota)
{
	...
	while (1) {
		...
		while ((skb = __skb_dequeue(&sd->process_queue))) {
			rcu_read_lock();
			local_irq_enable();
			__netif_receive_skb(skb, skb->protocol, skb_queue_len(&sd->input_pkt_queue));
			local_irq_disable();
			rcu_read_unlock();
			input_queue_head_incr(sd);
			if (++work >= quota) {
